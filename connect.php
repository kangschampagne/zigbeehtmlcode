<?php 
  ...
  //temp
    $temp_result = mysql_query("SELECT * FROM zigbee_temp WHERE temp_id IN (
        SELECT max(temp_id)  FROM zigbee_temp) ");
    //create an array
    $temp_response = array();
    $temp_posts = array();
  ...
  //gas
    ...
    $light_fp = fopen('json/lightintensity.json', 'w');
    fwrite($light_fp, json_encode($light_response));
    fclose($light_fp);
    //write to temp json file
    $temp_fp = fopen('json/temperature.json', 'w');
